...
//publishing all paths
    void Parser::publishRouteNetwork() {
        ...
        geometry_msgs::TransformStamped tf_to_rosstd;
        tf_to_rosstd.header.frame_id = "osm";
        tf_to_rosstd.child_frame_id = map_frame;
        tf_to_rosstd.transform.rotation.w = 0;
        tf_to_rosstd.transform.rotation.z = 1; 
        for (int i = 0; i < ways.size(); i++) {
            ...
            for (int j = 0; j < ways[i].nodesId.size(); j++) {
                ...
                //tf2::doTransform(pose, pose, tf_to_rosstd);
