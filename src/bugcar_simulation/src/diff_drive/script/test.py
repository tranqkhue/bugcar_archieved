...
class SpawnModel():
    def __init__(self):
        self.initial_latlon          = dict()
        self.initial_xyz             = [0,0,0]
        self.xyz_position            = False
        self.latlon_position         = False
    ...
    def parseUserInputs(self):
        ...
        for i in range(0,len(sys.argv)):
          ...
          if sys.argv[i] == '-lat':
            if self.xyz_position == True:
              rospy.logerr("Please use only lat-lon coordinate or xyz \n Already use xyz")
            else:
              self.latlon_position = True
              if len(sys.argv) > i+1:
                self.initial_latlon['lat'] = float(sys.argv[i+1])
          if sys.argv[i] == '-lon':
            if self.xyz_position == True:
              rospy.logerr("Please use only lat-lon coordinate or xyz \n Already use xyz")
            else:
              self.latlon_position = True
              if len(sys.argv) > i+1:
                self.initial_latlon['lon'] = float(sys.argv[i+1])
          if sys.argv[i] == '-x':
            if self.latlon_position ==True:
              rospy.logerr("Please use only lat-lon coordinate or xyz \n Already use lat-lon")
            else:
              self.xyz_position = True
              if len(sys.argv) > i+1:
        ...
        if self.model_name == "":
          ...
          sys.exit(0)
        if 'lat' in self.initial_latlon and 'lon' in self.initial_latlon:
          self.initial_xyz[:2] = utm.from_latlon(self.initial_latlon['lat'],\
                                                 self.initial_latlon['lon'])
        if not self.sdf_format and self.build_world:
    ...
    def callSpawnService(self):
        ...
        if self.latlon_position:
          rospy.loginfo(self.initial_xyz)
          self.initial_xyz[0], self.initial_xyz[1] = OsmParser.getUtmInWorld(self.initial_latlon['lat'], self.initial_latlon['lon'], self.osmFile)
          rospy.loginfo(self.initial_xyz)
        initial_pose = Pose()
        initial_pose.position.x = self.initial_xyz[1]
        initial_pose.position.y = self.initial_xyz[0]
